    ...
    if (view === 'month') {
      ...
      return (
        <>
          ...
          <span className="max-md:hidden">{format(currentDate, 'EEE MMMM d, yyyy', { locale: ptBR })}</span>
        </>
      );
    } else if (view === 'agenda') {
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="gap-1.5 max-[479px]:h-8">
                  <span>
                    ...
                    <span className="max-[479px]:sr-only">{formatText(view.charAt(0).toUpperCase() + view.slice(1))}</span>
                  </span>
                  <ChevronDownIcon className="-me-1 opacity-60" size={16} aria-hidden="true" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="min-w-32 font-mulish">
