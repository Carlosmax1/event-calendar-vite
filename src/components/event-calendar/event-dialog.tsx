...
export function EventDialog({ event, isOpen, onClose, onSave, onDelete }: EventDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px] font-mulish">
        <DialogHeader>
            ...
            <div className="flex-1 *:not-first:mt-1.5">
              ...
              <Popover open={endDateOpen} onOpenChange={setEndDateOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className={cn('truncate', !endDate && 'text-muted-foreground')}>{endDate ? format(endDate, 'PPP', { locale: ptBR }) : 'Pick a date'}</span>
                    <RiCalendarLine size={16} className="text-muted-foreground/80 shrink-0" aria-hidden="true" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-2" align="start">
