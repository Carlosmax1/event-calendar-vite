                            ...
                            <div key={`spanning-${event.id}-${day.toISOString().slice(0, 10)}`} className="aria-hidden:hidden" aria-hidden={isHidden ? 'true' : undefined}>
                              <EventItem onClick={(e) => handleEventClick(event, e)} event={event} view="month" isFirstDay={isFirstDay} isLastDay={isLastDay}>
                                <div className="invisible" aria-hidden={true}>
                                  {!event.allDay && <span>{format(new Date(event.start), 'HH:mm', { locale: ptBR })} </span>}
                                  {event.title}
                                </div>
                              </EventItem>
                            </div>
                        ...
                        <Popover modal>
                          ...
                          >
                            <div className="space-y-2">
                              <div className="text-sm font-medium">{format(day, 'EEE d', { locale: ptBR })}</div>
                              <div className="space-y-1">
                                {sortEvents(allEvents).map((event) => {
                                  ...
                                  return <EventItem key={event.id} onClick={(e) => handleEventClick(event, e)} event={event} view="month" isFirstDay={isFirstDay} isLastDay={isLastDay} />;
                                })}
                              </div>
                            </div>
                          </PopoverContent>
